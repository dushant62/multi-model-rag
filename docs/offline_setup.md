# Running Multi-Model-RAG in an Offline Environment

This document explains a critical consideration for running the Multi-Model-RAG project in an environment with no internet access.

## The Network Dependency: `LightRAG` and `tiktoken`

The `MultiModelRAG` core engine relies on the `LightRAG` library for its primary functionality. `LightRAG`, in turn, uses OpenAI's `tiktoken` library for text tokenization.
...
This dependency is indirect. The `Multi-Model-RAG` codebase itself does not directly import or call `tiktoken`. The call is made from within the `lightrag` library.
...
5.  **Run the application:** Start the `Multi-Model-RAG` application. For example:
    ```
    uv run examples/multi_model_rag_example.py requirements.txt
    ```

By following these steps, you can eliminate the network dependency and run the `Multi-Model-RAG` project successfully in a fully offline environment.
