...
- The custom parser registry is primarily intended for Python usage, where your
  ...
  before invoking MultiModelRAG APIs.
- The standalone CLI (``python -m multi_model_rag.parser`` or the installed console
...
from multi_model_rag.document_ir import ParsedDocument, ParserCapabilities
...
class Parser:
    ...
    def get_name(self) -> str:
        """Return the stable parser name used in metadata and caches."""
        return self.NAME

    def get_capabilities(self) -> ParserCapabilities:
        """Describe the parser's supported inputs and extraction strengths."""
        return ParserCapabilities(name=self.get_name())
    ...
    def build_parsed_document(
        ...
        """Normalize legacy parser output into the typed document IR."""
        return ParsedDocument.from_content_list(
            content_list,
            source_path=str(source_path),
            parser_name=self.get_name(),
            parser_capabilities=self.get_capabilities(),
            metadata=metadata,
        )
    ...
    def convert_office_to_pdf(
        ...
        try:
            ...
            cls.logger.error(f"Error in convert_office_to_pdf: {e!s}")
            raise
    ...
    def convert_text_to_pdf(
        ...
        try:
            ...
            cls.logger.error(f"Error in convert_text_to_pdf: {e!s}")
            raise
    ...
    def parse_office_doc(
        ...
        **kwargs,
        ...
        """Optional office-document parser hook."""
        raise NotImplementedError("parse_office_doc must be implemented by subclasses")
    ...
    def parse_html(
        ...
        **kwargs,
        ...
        """Optional HTML parser hook."""
        raise NotImplementedError("parse_html must be implemented by subclasses")
    ...
    def parse_text_file(
        ...
        **kwargs,
        ...
        """Optional plain-text parser hook."""
        raise NotImplementedError("parse_text_file must be implemented by subclasses")
...
class MineruParser(Parser):
    ...
    def get_capabilities(self) -> ParserCapabilities:
        return ParserCapabilities(
            ...
            provides_equations=True,
            provides_ocr=True,
            notes=("CLI and HTTP-client backends are supported through MinerU runtime options.",),
        )
...
class DoclingParser(Parser):
    ...
    def get_capabilities(self) -> ParserCapabilities:
        return ParserCapabilities(
            ...
            provides_ocr=True,
            notes=(
                "Docling preserves richer typed structure than the legacy content_list surface.",
            ),
        )
...
class PaddleOCRParser(Parser):
    ...
    def get_capabilities(self) -> ParserCapabilities:
        return ParserCapabilities(
            ...
            provides_reading_order=True,
            provides_ocr=True,
            notes=("Current integration is OCR-first and returns text-centric legacy blocks.",),
        )
...
def register_parser(name: str, parser_class: type) -> None:
    """Register a custom parser class for use with MultiModelRAG.
    ...
    Example::
        ...
        from multi_model_rag.parser import Parser, register_parser
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Python process are also accepted when you integrate MultiModelRAG "
