"""
Document processing functionality for MultiModelRAG
...
from multi_model_rag.document_ir import ParsedDocument
from multi_model_rag.parser import MineruExecutionError, MineruParser, Parser, get_parser
...
class ProcessorMixin:
    """ProcessorMixin class containing document processing functionality for MultiModelRAG"""
    ...
    def _build_parsed_document(
        ...
        **metadata,
        ...
        parser_instance = doc_parser or getattr(self, "doc_parser", None)
        parser_name = getattr(self.config, "parser", "unknown")
        parser_capabilities = None

        if parser_instance is not None:
            get_name = getattr(parser_instance, "get_name", None)
            if callable(get_name):
                parser_name = get_name()
            get_capabilities = getattr(parser_instance, "get_capabilities", None)
            if callable(get_capabilities):
                parser_capabilities = get_capabilities()
            build_parsed_document = getattr(parser_instance, "build_parsed_document", None)
            if callable(build_parsed_document):
        ...
        return ParsedDocument.from_content_list(
            content_list,
            source_path=str(file_path),
    ...
    def _parse_callable_accepts_kwarg(self, parse_callable: Any, kwarg_name: str) -> bool:
        try:
    ...
    def _parser_overrides_method(self, parser_instance: Any, method_name: str) -> bool:
        parser_method = getattr(type(parser_instance), method_name, None)
        base_method = getattr(Parser, method_name, None)
        return parser_method is not None and parser_method is not base_method

    async def _parse_document_for_workflow(
        self,
        file_path: str,
        output_dir: str = None,
        ...
        **kwargs,
    ) -> tuple[List[Dict[str, Any]], ParsedDocument, str]:
        ...
        parse_callable = self.parse_document
        parse_kwargs = {
            "file_path": file_path,
        ...
        if self._parse_callable_accepts_kwarg(parse_callable, "emit_callbacks"):
            parse_kwargs["emit_callbacks"] = False

        try:
            content_list, doc_id = await parse_callable(**parse_kwargs)
        ...
        if callback_manager is not None:
            callback_manager.dispatch(
                "on_parse_complete",
                file_path=str(file_path),
                content_blocks=parsed_document.block_count,
                doc_id=doc_id,
                duration_seconds=time.time() - parse_start_time,
            )

        return content_list, parsed_document, doc_id
    ...
    async def _run_parser_for_file(
        ...
        **kwargs,
        ...
        get_capabilities = getattr(doc_parser, "get_capabilities", None)
        capabilities = get_capabilities() if callable(get_capabilities) else None
        html_formats = set(getattr(doc_parser, "HTML_FORMATS", (".html", ".htm", ".xhtml")))

        if ext == ".pdf":
            if self._parser_overrides_method(doc_parser, "parse_pdf"):
        ...
        if ext in Parser.IMAGE_FORMATS:
            self.logger.info("Detected image file, using parser for images...")
            if (capabilities is None or capabilities.supports_images) and (
                self._parser_overrides_method(doc_parser, "parse_image")
            ):
                try:
                    return await asyncio.to_thread(
        ...
        if (
            ext in Parser.OFFICE_FORMATS
            and (capabilities is None or capabilities.supports_office)
            and self._parser_overrides_method(doc_parser, "parse_office_doc")
        ):
        ...
        if (
            ext in html_formats
            and (capabilities is None or capabilities.supports_html)
            and self._parser_overrides_method(doc_parser, "parse_html")
        ):
        ...
        if (
            ext in Parser.TEXT_FORMATS
            and (capabilities is None or capabilities.supports_text)
            and self._parser_overrides_method(doc_parser, "parse_text_file")
        ):
            self.logger.info("Detected text document, using parser for text files...")
            return await asyncio.to_thread(
        ...
        try:
            ...
            cache_data = {
                cache_key: {
                    ...
                    "parsed_document": (
                        parsed_document.to_dict() if parsed_document is not None else None
                    ),
                    "doc_id": doc_id,
                    "mtime": file_mtime,
    ...
    async def parse_document(
        ...
        **kwargs,
        ...
        callback_manager = getattr(self, "callback_manager", None) if emit_callbacks else None
        parse_start_time = time.time()
        if callback_manager is not None:
            callback_manager.dispatch(
                "on_parse_start",
        ...
        try:
            ...
            content_list = await self._run_parser_for_file(
                doc_parser,
                file_path=file_path,
                output_dir=output_dir,
                parse_method=parse_method,
            ...
            self.logger.error(f"Error during parsing with {self.config.parser} parser: {e!s}")
            if callback_manager is not None:
                callback_manager.dispatch(
                    "on_parse_error",
                    file_path=callback_file,
        ...
        if display_stats:
            self.logger.info("\nContent Information:")
            self.logger.info(f"* Total blocks in content_list: {parsed_document.block_count}")

            self.logger.info("* Content block types:")
            for block_type, count in parsed_document.block_type_counts.items():
    ...
    def _apply_chunk_template(
        ...
        try:
            if content_type == "image":
                ...
                captions = original_item.get("image_caption", original_item.get("img_caption", []))
    ...
    async def process_document_complete(
        ...
        **kwargs,
        ...
        try:
            ...
            # Step 1: Parse document through the workflow wrapper so the
            # orchestration layer owns parse lifecycle callbacks.
            (
                content_list,
                parsed_document,
                content_based_doc_id,
            ) = await self._parse_document_for_workflow(
            ...
            text_content, multimodal_items = separate_content(parsed_document)
            ...
            if hasattr(self, "set_content_source_for_context") and multimodal_items:
                ...
                self.set_content_source_for_context(
                    parsed_document.to_content_list(), self.config.content_format
                )
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Error processing document {file_path}: {e!s}")
                return False
            ...
            self.logger.error(f"Error processing document {file_path}: {e!s}")
            self.logger.debug("Exception details:", exc_info=True)
            ...
            if pipeline_status_lock and pipeline_status:
                try:
                    async with pipeline_status_lock:
                        ...
                        error_msg = f"MultiModelRAG processing failed for {file_name}: {e!s}"
            ...
            async with pipeline_status_lock:
                ...
                pipeline_status["latest_message"] = (
                    f"MultiModelRAG processing completed for {file_name}"
                )
    ...
    async def insert_content_list(
        ...
        parsed_document = self._build_parsed_document(content_list, file_path=file_path)
