...
from multi_model_rag.batch import BatchMixin
from multi_model_rag.callbacks import CallbackManager
...
from multi_model_rag.parser import SUPPORTED_PARSERS, MineruParser, get_parser
from multi_model_rag.processor import ProcessorMixin
from multi_model_rag.query import QueryMixin
from multi_model_rag.utils import get_processor_supports


@dataclass
class MultiModelRAG(QueryMixin, ProcessorMixin, BatchMixin):
    ...
    config: Optional[MultiModelRAGConfig] = field(default=None)
    ...
    def __post_init__(self):
        ...
        self.logger.info("MultiModelRAG initialized with config:")
    ...
    def _create_context_config(self) -> ContextConfig:
        """Create context configuration from MultiModelRAG config"""
    ...
    async def _ensure_lightrag_initialized(self, skip_parser_installation_check: bool = False):
        """Ensure LightRAG instance is initialized, create if necessary"""
        try:
            # Check parser installation first
            if not skip_parser_installation_check and not self._parser_installation_checked:
            ...
            if self.lightrag is not None:
                ...
                try:
                    ...
                    error_msg = f"Failed to initialize pre-provided LightRAG instance: {e!s}"
            ...
            try:
                ...
                # Wire FlagEmbedding reranker if enabled and not already supplied
                if (
                    self.config
                    and self.config.enable_reranker
                    and "rerank_model_func" not in self.lightrag_kwargs
                ):
                    try:
                        from multi_model_rag.rerank import create_flagembedding_reranker

                        reranker = create_flagembedding_reranker(
                            model_name=self.config.reranker_model
                        )
                        self.lightrag.rerank_model_func = reranker
                        self.lightrag.min_rerank_score = self.config.min_rerank_score
                        self.logger.info(
                            "FlagEmbedding reranker wired (%s, min_score=%.2f)",
                            self.config.reranker_model,
                            self.config.min_rerank_score,
                        )
                    except Exception as _re:
                        self.logger.info("Reranker not available, skipping: %s", _re)
                ...
                error_msg = f"Failed to initialize LightRAG instance: {e!s}"
                self.logger.error(error_msg, exc_info=True)
                return {"success": False, "error": error_msg}

        except Exception as e:
            error_msg = f"Unexpected error during LightRAG initialization: {e!s}"
            self.logger.error(error_msg, exc_info=True)
            return {"success": False, "error": error_msg}
    ...
    async def finalize_storages(self):
        ...
        Example usage:
            try:
                ...
                await multi_model_rag.process_file("document.pdf")
                # ... other operations ...
            finally:
                # Always finalize storages to clean up resources
                if multi_model_rag:
                    await multi_model_rag.finalize_storages()
        ...
        try:
            ...
            if tasks:
                ...
                self.logger.info("Successfully finalized all MultiModelRAG storages")
            else:
