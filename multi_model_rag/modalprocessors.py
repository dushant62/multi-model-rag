...
@dataclass
class StructuredModalAnalysis:
    """Normalized modal analysis result used across all modal processors."""

    detailed_description: str
    entity_info: Dict[str, Any]
    retrieval_hints: Dict[str, List[str]] = field(default_factory=dict)
    parse_strategy: str = "json"

    def to_metadata(self) -> Dict[str, Any]:
        return {
            "parse_strategy": self.parse_strategy,
            "retrieval_hints": {key: list(value) for key, value in self.retrieval_hints.items()},
        }

    def __iter__(self):
        yield self.detailed_description
        yield self.entity_info
...
class BaseModalProcessor:
    ...
    async def _create_entity_and_chunk(
        ...
        modal_metadata = entity_info.get("structured_analysis", {})
        retrieval_hints = modal_metadata.get("retrieval_hints", {})
        retrieval_hint_lines = self._format_retrieval_hints(retrieval_hints)
    ...
    @staticmethod
    def _summarize_description_points(description: str, limit: int = 3) -> List[str]:
        if not description:
            return []
        ...
        for segment in segments:
            ...
            if clean_segment and clean_segment not in points:
                points.append(clean_segment)
            if len(points) >= limit:
                break
        return points

    def _build_retrieval_hints(
        self, response_data: Dict[str, Any], description: str
    ) -> Dict[str, List[str]]:
        raw_hints = response_data.get("retrieval_hints", {})
        if not isinstance(raw_hints, dict):
            raw_hints = {}

        retrieval_hints = {
            "keywords": self._normalize_string_list(raw_hints.get("keywords"), limit=8),
            "observations": self._normalize_string_list(raw_hints.get("observations"), limit=5),
            "relationships": self._normalize_string_list(raw_hints.get("relationships"), limit=3),
            "context_signals": self._normalize_string_list(
                raw_hints.get("context_signals"), limit=3
            ),
        }

        if not retrieval_hints["observations"]:
            retrieval_hints["observations"] = self._summarize_description_points(
                description, limit=3
            )

        if not retrieval_hints["keywords"]:
            keyword_candidates = set()
            for point in retrieval_hints["observations"]:
                for token in re.findall(r"[A-Za-z0-9][A-Za-z0-9_\-/]{2,}", point):
                    keyword_candidates.add(token.lower())
            retrieval_hints["keywords"] = list(sorted(keyword_candidates))[:8]

        return retrieval_hints

    def _build_structured_analysis(
        self, response: str, entity_name: str | None, content_type: str
    ) -> StructuredModalAnalysis:
        try:
            response_data = self._robust_json_parse(response)
            description = str(response_data.get("detailed_description", "")).strip()
            entity_data = response_data.get("entity_info", {})

            if not description or not isinstance(entity_data, dict):
            ...
            normalized_entity = {
                ...
                if entity_name
                else f"{str(entity_data['entity_name']).strip()} ({str(entity_data['entity_type']).strip()})",
                "entity_type": str(entity_data["entity_type"]).strip() or content_type,
                "summary": str(entity_data["summary"]).strip(),
            }
            return StructuredModalAnalysis(
                detailed_description=description,
                entity_info=normalized_entity,
                retrieval_hints=self._build_retrieval_hints(response_data, description),
                parse_strategy="json",
            )
            ...
            return StructuredModalAnalysis(
                ...
                retrieval_hints={
                    "keywords": self._normalize_string_list(content_type, limit=1),
                    "observations": self._summarize_description_points(cleaned, limit=3),
                    "relationships": [],
                    "context_signals": [],
                },
    ...
    @staticmethod
    def _format_retrieval_hints(retrieval_hints: Dict[str, List[str]]) -> str:
        sections = []
        for label, key in [
            ("Keywords", "keywords"),
            ...
            values = retrieval_hints.get(key, [])
            if values:
    ...
    def _extract_fields_with_regex(self, response: str) -> dict:
        ...
        def extract_list_field(field_name: str) -> list[str]:
            list_match = re.search(
                rf'"{field_name}":\s*\[(.*?)\]', response, re.DOTALL | re.IGNORECASE
            )
            if not list_match:
                return []
            return self._normalize_string_list(
                re.findall(r'"([^"]*(?:\\.[^"]*)*)"', list_match.group(1))
            )
        ...
        return {
            ...
            "retrieval_hints": {
                "keywords": extract_list_field("keywords"),
                "observations": extract_list_field("observations"),
                "relationships": extract_list_field("relationships"),
                "context_signals": extract_list_field("context_signals"),
            },
        }
    ...
    async def _process_chunk_for_extraction(
        ...
        chunk_vdb_data = {
            chunk_id: {
                ...
                "modal_metadata": chunk_data.get("modal_metadata", {}),
            }
        }
...
class ImageModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            analysis = self._parse_response(response, entity_name)
            entity_info = dict(analysis.entity_info)
            entity_info["structured_analysis"] = analysis.to_metadata()

            return analysis.detailed_description, entity_info
    ...
    async def process_multimodal_content(
        ...
        try:
            ...
            structured_hints = self._format_retrieval_hints(
                entity_info.get("structured_analysis", {}).get("retrieval_hints", {})
            )
    ...
    def _parse_response(self, response: str, entity_name: str = None) -> StructuredModalAnalysis:
        """Parse image analysis response into structured analysis."""
        return self._build_structured_analysis(response, entity_name, "image")
...
class TableModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            analysis = self._parse_table_response(response, entity_name)
            entity_info = dict(analysis.entity_info)
            entity_info["structured_analysis"] = analysis.to_metadata()
    ...
    def _parse_table_response(
        ...
        """Parse table analysis response into structured analysis."""
        return self._build_structured_analysis(response, entity_name, "table")
...
class EquationModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            analysis = self._parse_equation_response(response, entity_name)
            entity_info = dict(analysis.entity_info)
            entity_info["structured_analysis"] = analysis.to_metadata()
    ...
    def _parse_equation_response(
        ...
        """Parse equation analysis response into structured analysis."""
        return self._build_structured_analysis(response, entity_name, "equation")
...
class GenericModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            analysis = self._parse_generic_response(response, entity_name, content_type)
            entity_info = dict(analysis.entity_info)
            entity_info["structured_analysis"] = analysis.to_metadata()
    ...
    def _parse_generic_response(
        ...
        """Parse generic analysis response into structured analysis."""
        return self._build_structured_analysis(response, entity_name, content_type)
